...
public abstract class Jogador {
    ...
    public Jogador(String nome, double hp, int ataque, int defesa, int destreza, int velocidade, Log log) {
        setNome(nome);
        setHp(hp);
        setAtaque(ataque);
        setDefesa(defesa);
     ...
     */
    public static Jogador gerarJogadorAleatorio(Dificuldade dificuldade, Log log) {
        ...
        switch (nome) {
            case "Guerreiro":
                jogadorCriado = new Guerreiro(nome, 120, 30, 25, 10, 20, random.nextInt(25) + 1, random.nextInt(20) + 1, log);
                break;
            case "Mago":
                jogadorCriado = new Mago(nome, 80, 40, 10, 25, 15, random.nextInt(50) + 30, log);
                break;
            case "Arqueiro":
                jogadorCriado = new Arqueiro(nome, 90, 35, 15, 30, 25, random.nextInt(100) + 1, random.nextInt(20) + 1, log);
                break;
            case "Goblin":
                jogadorCriado = new Goblin(nome, 50, 15, 5, 20, 30, random.nextInt(5) + 1, log);
                break;
            case "Lobisomem":
                jogadorCriado = new Lobisomem(nome, 120, 35, 20, 30, 35, random.nextInt(20) + 1, random.nextInt(15) + 1, log);
                break;
            case "Dragão":
                jogadorCriado = new Dragao(nome, 200, 50, 40, 10, 10, log);
                break;
        }
    ...
    public static Jogador gerarJogadorEspecifico(Class<? extends Jogador> tipo, Dificuldade dificuldade, Log log) {
        Random random = new Random();
        ...
        if (tipo == Dragao.class) {
            ...
            return new Dragao("Dragão", hpBase, ataqueBase, defesaBase, destrezaBase, velocidadeBase, log);
        }
